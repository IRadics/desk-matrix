const smallScrew: AdditionalPart = {
    ...
    description: 'For securing the beams to each other. Flat t-bolt is recommended, but any 9mm long small bolt works.',
    quantity: 1,
...
const largeScrew: AdditionalPart = {
    ...
    description: 'For securing the clamp to the desk. The small hole at the end is important for the desk protector plate.',
    quantity: 1,
...
const bottomProtector: AdditionalPart = {
    ...
    description: 'For protecting the bottom of the desk by distributing the weight and clamping force',
    quantity: 1,
}

const topProtector: AdditionalPart = {
    partName: 'Top desk protector',
    description: 'For protecting the top of the desk by distributing the weight and clamping force',
    quantity: 1,
}
...
const addPart = (parts: AdditionalPart[], part: AdditionalPart, quantity: number = 1) =>{
...
export const getAdditionalparts = (parts: Part[]) : {
    bolts: AdditionalPart[],
    other: AdditionalPart[]
} =>{
    let bolts : AdditionalPart[] = []
    let other : AdditionalPart[] = []
    parts.forEach((part)=>{
        switch(part.partType){
            case 'beam':{
                if(part.beamType === 'female-male' || part.beamType === 'male-female'){
