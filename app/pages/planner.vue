...
defineSoftwareApp({
  name: 'DeskMatrix Planner',
  description:
    'Plan a custom DeskMatrix layout and calculate the beams, corners, clamps, boards and bolts required for your desk-mounted MultiBoard.',
  icon: '/thumbnail.png',
  url: 'https://deskmatrix.io/planner',
  version: '1.0.0',
})
...
useSeoMeta({
  ...
  ogTitle:
    'DeskMatrix Planner - Plan your DeskMatrix layout and 3D print the required parts',
  description:
    'Plan a custom DeskMatrix layout and calculate the beams, corners, clamps, boards and bolts required for your desk-mounted MultiBoard.',
  ogDescription:
    'Plan a custom DeskMatrix layout and calculate the beams, corners, clamps, boards and bolts required for your desk-mounted MultiBoard.',
...
const connectors: Part['beamType'][] = [
  'male-male',
  'male-female',
  'female-male',
  'female-female',
...
const addPart = (part: AddPartData, position?: Vector2d) => {
  const initialPosition =
    position ??
    (part.partType === 'board'
      ? boardModalClickPos.value
      : contextMenuClickPos.value)
...
const onContextMenuOpen = (open: boolean) => {
  ...
  contextMenuClickPos.value =
    canvas.value?.stageNode?.getRelativePointerPosition() ?? null
}
...
onBeforeRouteLeave(async () => {
  if (parts.value.length > 0) {
    const confirmLeave = window.confirm(
      'The changes you have made will be lost. Are you sure you want to leave?',
    )
    if (!confirmLeave) {
...
<template>
  <main>
    <div class="bg-neutral-500">
      <SideBar v-model="sideBarOpen" :parts="parts" />
      <div
        class="flex gap-2 h-(--ui-header-height) bg-neutral-900 items-center px-2 md:px-8 justify-center"
      >
        <NuxtLink
          class="mr-auto shrink-0 hidden md:block"
          to="/"
          aria-label="DeskMatrix home"
        >
          <UIcon class="h-[60px]" :size="60" name="i-custom-logo" />
        </NuxtLink>
        <NuxtLink
          class="mr-auto shrink-0 block md:hidden"
          to="/"
          aria-label="DeskMatrix home"
        >
        ...
        <template #content>
          <div class="bg-neutral-900/50 flex flex-col gap-2 px-4 py-2">
            ...
            <URadioGroup
              ...
              @update:model-value="
                v => setBoardVisibility((v as Visibility) ?? Visibility.normal)
              "
            />
            ...
            <URadioGroup
              ...
              :items="visibilityItems"
              @update:model-value="
                v => setBeamVisibility((v as Visibility) ?? Visibility.normal)
              "
            />
          </div>
        </template>
      ...
      <UCollapsible
        class="absolute top-(--ui-header-height) z-50 right-0 flex flex-col w-50"
      >
        <UButton
          label="Help"
          color="neutral"
        ...
        <template #content>
          <div class="bg-neutral-900/50 flex flex-col gap-1 px-4 py-2">
            ...
            <div class="flex items-center">
              <UIcon
                name="i-lucide-mouse-left"
                class="[&_circle]:text-primary-500"
                :size="24"
              />
            ...
            <ul class="list-inside list-disc text-sm flex flex-col gap-1">
              <li>
                Clicking on a snapped group twice selects the individual part,
                which you can disconnect by dragging it away
              </li>
              <li>
                Currently you can only connect individual parts to other parts
                or groups. Group -> Group connection is not implemented yet
              </li>
            </ul>
          </div>
        </template>
