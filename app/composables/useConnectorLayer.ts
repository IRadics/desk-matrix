...
export const useConnectorLayer = (
  partGroupNode?: RefLike<Group>,
  ...
  const setConnectorLayer = () => {
    ...
    connectorNodes.forEach(conNode => {
      ...
      connectors.value.push({
        ...
        offsetPosition: {
          x:
            conNode.getAbsolutePosition().x -
            partGroupNode.value.getAbsolutePosition().x,
          y:
            conNode.getAbsolutePosition().y -
            partGroupNode.value.getAbsolutePosition().y,
        },
      })
    })
    connectors.value.forEach(con => {
