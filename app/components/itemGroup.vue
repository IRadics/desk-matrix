...
import { Group as VGroup, type VueKonvaRef } from 'vue-konva'

const recoverPosition = (part: Group) => {
  const stagePos = part.getStage()?.getAbsolutePosition() ?? { x: 0, y: 0 }
  part.setAbsolutePosition({
    x:
      part.getAbsolutePosition().x +
      (groupNode.value?.getAbsolutePosition().x ?? 0) -
      stagePos.x,
    y:
      part.getAbsolutePosition().y +
      (groupNode.value?.getAbsolutePosition().y ?? 0) -
      stagePos.y,
  })
}
...
onMounted(() => {
  ...
  groupData.value.forEach(d =>
    d.componentRef.setConnectorLayer
      ? d.componentRef.setConnectorLayer()
      : null,
  )
  moveBoardsToTop()
...
onBeforeUnmount(() => {
  ...
  groupData.value.forEach(d =>
    d.componentRef.unSetConnectorLayer
      ? d.componentRef.unSetConnectorLayer()
      : null,
  )
})
...
const onDrag = () => {
  groupData.value
    .filter(d => Boolean(d.componentRef.updateConnectorPositions))
    .forEach(d => {
