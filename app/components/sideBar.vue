...
const partsGroupedDmCount = computed(
  () => partsGroupedDm.value.flatMap(v => v).length,
)
const partsGroupedMbCount = computed(
  () => partsGroupedMb.value.flatMap(v => v).length,
)
const boltCount = computed(() =>
  additionalParts.value.bolts.reduce((acc, value) => {
    return acc + value.quantity
  }, 0),
)
const otherCount = computed(() =>
  additionalParts.value.other.reduce((acc, value) => {
    return acc + value.quantity
  }, 0),
)
  ...
  <div class="flex flex-1 sidebar-w">
    <USidebar
      ...
      :ui="{
        root: 'block!',
        container:
          'absolute top-(--ui-header-height) bottom-0 h-[calc(100%-var(--ui-header-height))] block!',
    ...
    >
      <template #header="{ close, open }">
        <div
          v-if="$device.isMobileOrTablet && !hideMobileWarning"
          class="absolute right-0 top-0 translate-x-full flex justify-center items-center transition-all bg-red-500/20"
          :class="infobarWidthClass"
        >
          <span class="text-sm px-4"
            >The planner application is not yet optimized for mobile devices. It
            may not work as intended</span
          >
          <UButton
            aria-label="Dismiss mobile compatibility warning"
            icon="i-lucide-x"
            color="neutral"
            size="xl"
            variant="link"
            @click="hideMobileWarning = true"
          />
        </div>
        <div
          class="absolute right-0 bottom-0 translate-x-full flex justify-center transition-all bg-black/20"
          :class="infobarWidthClass"
        >
          <span class="text-sm w-full px-4"
            >Unofficial community tool. Not affiliated with Multiboard
            LTD.</span
          >
        </div>
        <div class="flex w-full items-center gap-4">
          ...
          <UButton
            v-if="open"
            aria-label="Collapse bill of materials sidebar"
          ...
          <UButton
            v-if="!open"
            aria-label="Expand bill of materials sidebar"
      ...
      <template v-if="sideBarOpen">
        ...
        >
          ...
          <template #dm-parts>
            <div class="divide-y divide-default">
              <template v-if="partsGroupedDm.length">
                <SideBarPart
                  v-for="(groupedParts, index) in partsGroupedDm"
                  :key="groupedParts[0]?.id ?? index"
                  :parts="groupedParts"
                />
              </template>
              <template v-else>
          ...
          <template #multiboard-parts>
            <div class="divide-y divide-default">
              <template v-if="partsGroupedMb.length">
                <SideBarPart
                  v-for="(groupedParts, index) in partsGroupedMb"
                  :key="groupedParts[0]?.id ?? index"
                  :parts="groupedParts"
                />
              </template>
          ...
          <template #bolts>
            <div class="italic px-2 text-sm">
              <strong>NOTE:</strong> the calculation assumes that all male
              connectors are used
            </div>
            <div class="divide-y divide-default">
              <template v-if="boltCount > 0">
                <AdditionalPart
                  v-for="part in additionalParts.bolts"
                  :key="part.partName"
                  :part="part"
                />
              </template>
          ...
          <template #other>
            <div class="divide-y divide-default">
              <template v-if="otherCount > 0">
                <AdditionalPart
                  v-for="part in additionalParts.other"
                  :key="part.partName"
                  :part="part"
                />
              </template>
