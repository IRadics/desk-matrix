...
const canConnectByDirection = (
  d1: SnapPoint['direction'],
  d2: SnapPoint['direction'],
  t1: SnapPoint['type'],
  t2: SnapPoint['type'],
) => {
  if (
...
const activeBaseSnapPoint = ref<{
  partId: string
  snapPoint: SnapPoint
} | null>(null)
const activePartSnapPoint = ref<{
  partId: string
  snapPoint: SnapPoint
} | null>(null)

const searchConnection = useThrottleFn(
  (basePartId: string, excludeBoard?: boolean) => {
    ...
    basePartSnapPoints.forEach(snapPoint => {
      partRefs.value
        ?.filter(part => part.id !== basePartId)
        .forEach(part => {
          const partSnapPoints = part
            .getSnapPoints()
            .filter(
              partSnapPoint =>
                canConnectByDirection(
                  snapPoint.direction,
                  partSnapPoint.direction,
                  snapPoint.type,
                  partSnapPoint.type,
                ) &&
                canConnectByType(snapPoint.type, partSnapPoint.type) &&
                (!excludeBoard || part.partType !== 'board'),
            )
          ...
          const snapPointsSorted = partSnapPoints.toSorted((a, b) => {
            const distance1 = Math.sqrt(
              Math.pow(a.x - snapPoint.x, 2) + Math.pow(a.y - snapPoint.y, 2),
            )
            const distance2 = Math.sqrt(
              Math.pow(b.x - snapPoint.x, 2) + Math.pow(b.y - snapPoint.y, 2),
            )
          ...
          const distance = Math.sqrt(
            Math.pow(closest.x - snapPoint.x, 2) +
              Math.pow(closest.y - snapPoint.y, 2),
          )
    ...
    if (closestDistance <= snapDistance && closestPart) {
      ...
      activeBaseSnapPoint.value = {
        partId: basePartId,
        snapPoint: closestBaseSnap!,
      }
      activePartSnapPoint.value = {
        partId: closestPart.id,
...
const selectedPartState = computed(() => {
  return {
    rotation:
      partRefs.value?.find(ref => ref.id === selectedPart.value)?.rotation ?? 0,
  }
})
...
const removePartFromGroup = (id: string) => {
  ...
  if (groupName) {
    ...
    groups.value[groupName] = groups.value[groupName].filter(
      g => g.part.id !== selectedPart.value,
    )

    if ((groups.value[groupName].length ?? 0) <= 1) {
      groups.value = Object.fromEntries(
        Object.entries(groups.value).filter(([key]) => key !== groupName),
      )
      parts.value
        .filter(part => part.group === groupName)
        .forEach(part => {
...
const snapPart = (
  basePart: PartInstance,
  targetPart: PartInstance,
  baseSnapPoint: SnapPoint,
...
const onGroupDragging = (isDragging: boolean, group: GroupData) => {
  if (
    ...
    group.some(p => p.part.id === selectedPart.value)
  ) {
...
const onDragging = (
  isDragging: boolean,
  partId: string,
  disableHighlightSearch?: boolean,
) => {
  if (isDragging && !disableHighlightSearch) {
    ...
    const basePart = partRefs.value?.find(
      part => part.id === activeBaseSnapPoint.value?.partId,
    )
    if (!basePart) {
      return
    }
    const snapToPart = partRefs.value?.find(
      part => part.id === activePartSnapPoint.value?.partId,
    )
    if (!snapToPart) {
      return
    }
    ...
    snapPart(
      basePart,
      snapToPart,
      activeBaseSnapPoint.value.snapPoint,
      activePartSnapPoint.value.snapPoint,
    )
    ...
    const closestPartData = parts.value.find(
      part => part.id === activePartSnapPoint.value?.partId,
    )
    if (!closestPartData) {
    ...
    groups.value[groupName] = [
      { part: basePartData, node: basePart.node, componentRef: basePart },
      {
        part: closestPartData!,
        node: snapToPart?.node,
        componentRef: snapToPart,
      },
    ]
...
const canDeleteSelectedPart = computed(() => {
  return Boolean(
    (selectedPart.value && !selectedItemGroup.value) ||
    (!selectedPart.value && selectedItemGroup.value),
  )
})
...
const deleteSelectedPart = async () => {
  if (selectedPart.value && !selectedItemGroup.value) {
    ...
    if (
      ...
      const partsFiltered = parts.value.filter(
        p => p.group !== selectedItemGroup.value,
      )
      parts.value
        .filter(p => p.group === selectedItemGroup.value)
        .forEach(p => {
...
onKeyStroke('Delete', () => deleteSelectedPart())
...
const addPart = (part: AddPartData | Part) => {
  if (!part.initialPosition) {
    const pos = stageNode.value
      ? screenCenterToStagePosition(stageNode.value)
      : { x: 300, y: 300 }
...
const mouseCursor = computed(() => {
  ...
  if (
    selectedItemGroup.value &&
    groups.value[selectedItemGroup.value]?.find(
      p => p.part.id === mouseOverPart.value,
    )
  ) {
    return 'grab'
  }
        ...
        <v-group ref="mainGroup">
          <template v-for="part in parts" :key="part.id">
            <ItemBeam
              ...
              :selected="
                selectedPart === part.id || selectedItemGroup === part.group
              "
            ...
            <ItemCorner
              v-if="
                (part.partType === '4-way-corner' ||
                  part.partType === '3-way-corner' ||
                  part.partType === '2-way-corner') &&
                part.snapType !== 'quad'
              "
              :id="part.id"
              ref="partRef"
